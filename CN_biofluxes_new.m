...
function [Bioflux] = CN_biofluxes_new (Clnew, dz, deltaz, D, BC)

nlayer = length(Clnew(:,1));
...
% with information of flux from above, and second column shows the flux 
% from below
...
delz2 = deltaz(1);
Bioflux(1,1) =  BC(1);
Bioflux(1,2) = (Clnew(2)-Clnew(1))*D2/(delz2/1000);
...
for ii=2:1:nlayer-1
    ...
    delz1 = deltaz(ii-1);
    delz2 = deltaz(ii);
    Bioflux(ii,1) = -(Clnew(ii)-Clnew(ii-1))*D1/(delz1/1000);
    Bioflux(ii,2) = (Clnew(ii+1)-Clnew(ii))*D2/(delz2/1000);
end
...
D1 = D(nlayer-1);
delz1 = deltaz(nlayer-1);
Bioflux(nlayer,1) =  -(Clnew(nlayer)-Clnew(nlayer-1))*D1/(delz1/1000);
Bioflux(nlayer,2) = -BC(nlayer);
